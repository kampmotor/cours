...
# 添加源代码路径
sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'python_mult_agent_sim', 'src'))

from utils import calculate_polarization, clamp_vector
...
def run_performance_analysis():
    ...
    for n_obstacles in obstacle_counts:
        ...
        for trial in range(n_trials):
            ...
            # 创建多障碍物环境
            env = MultiObstacleEnvironment(field_size=30)
            ...
            # 运行仿真
            robot_positions = simulator.robot_positions.copy()
            robot_velocities = simulator.robot_velocities.copy()
            ...
            # 存储轨迹
            robot_trajectory = np.zeros((simulator.n_robots, 2, T))
            robot_velocities_history = np.zeros((simulator.n_robots, 2, T))
            polarization_history = np.zeros(T)
            ...
            for t in range(T):
                ...
                for i in range(simulator.n_robots):
                    # 获取邻居
                    distances = np.linalg.norm(robot_positions - robot_positions[i], axis=1)
                    distances[i] = float('inf')  # 排除自己
                    sorted_indices = np.argsort(distances)
                    num_neighbors = min(k_neighbors, simulator.n_robots - 1)
                    selected_neighbors_i = sorted_indices[:num_neighbors]
                ...
                for i in range(simulator.n_robots):
                    ...
                    if new_positions[i, 0] < boundary_buffer:
                        new_positions[i, 0] = boundary_buffer
                        new_velocities[i, 0] = abs(new_velocities[i, 0]) * 0.5
                    elif new_positions[i, 0] > simulator.field_size - boundary_buffer:
                        new_positions[i, 0] = simulator.field_size - boundary_buffer
                        new_velocities[i, 0] = -abs(new_velocities[i, 0]) * 0.5
                    ...
                    if new_positions[i, 1] < boundary_buffer:
                        new_positions[i, 1] = boundary_buffer
                        new_velocities[i, 1] = abs(new_velocities[i, 1]) * 0.5
                    elif new_positions[i, 1] > simulator.field_size - boundary_buffer:
                        new_positions[i, 1] = simulator.field_size - boundary_buffer
                        new_velocities[i, 1] = -abs(new_velocities[i, 1]) * 0.5
